...
[#assign SecureIPs = ["GPIO","IPCC","RTC", "RAMCFG", "HPDMA", "RETRAM" ]]
[#function isMP2_A35TD_Or_M33TDNonSecureClock ipName]
	[#assign generate=true]
    [#if (ContextProject?? && !ContextProject?matches("A35TD_TZ(EN|DIS)"))]
        [#list SecureIPs as SecureIP]
            [#if ipName?contains(SecureIP)]
...
[#if config.comments?? && config.comments!=""]#n[#if nTab==2]#t#t[#else]#t[/#if]/** ${config.comments?replace("#t","#t")}#n[#if nTab==2]#t#t[#else]#t[/#if]*/[/#if]
[#else]
...
[/#if]
    ...
    [#list methodList as method][#assign args = ""]
            [#if method.name?matches("HAL_PLAY_(((IN|OUT)PUT)|(LUT))_SetConfig") && method.name != playMethod]
                [#assign playIndex = 0]
                [#assign playInputs = getPlayNumberOfMethods(configModel, method.name)]
            [/#if]
            [#if method.hardCode??] [#-- Hard code --]              
                ${method.hardCode.text?replace("$IpInstance",inst)} 
            [#else]
...
[#if ((config.name == "ADC_RegularChannelConfig" && (FamilyName!="STM32C0" || FamilyName!="STM32WBA" || FamilyName!="STM32G0" || FamilyName!="STM32WL"|| FamilyName!="STM32U5") ) || config.name == "ADC_InjectedChannelConfig" ||  ((FamilyName=="STM32C0" || FamilyName=="STM32G0" || FamilyName=="STM32WBA" || FamilyName=="STM32WL" || FamilyName=="STM32U5") && config.name == "ADC_RegularChannelRankConfig")) && (FamilyName!="STM32F0" && FamilyName!="STM32L0" && FamilyName!="STM32F2" && FamilyName!="STM32F4")]                                        [#list argument?keys as k]
                                    ...
                                    [#if instanceIndex??&&fargument.context=="global" && fargument.optional!="output"][#assign varName=fargument.name + link +instanceIndex][#else][#assign varName=fargument.name][/#if]
                                        ...
                                        [#if !listofDeclaration?contains(indicator) || method.name?matches("HAL_PLAY_(((IN|OUT)PUT)|(LUT))_SetConfig")][#-- if not repeted --]
                                           [#if !(argument.variableName)?contains("Nonce") && ( FamilyName !="STM32N6" || (FamilyName=="STM32N6" &&  (Secure=="true" || (Secure=="false" && argument.name != "SrcSecure"&& argument.name != "DestSecure"))))] [#if nTab==2]#t#t[#else]#t[/#if][#if instanceIndex??&&fargument.context=="global" && fargument.optional!="output"][#assign varName=fargument.name +link +instanceIndex]${fargument.name}${instanceIndex}[#else][#assign varName=fargument.name]${fargument.name}[#if method.name?matches("HAL_PLAY_(((IN|OUT)PUT)|(LUT))_SetConfig")][${playIndex}][/#if][/#if]${link}${argument.name} = ${argValue};[/#if]
...
[#assign index1 =index1 + 1]
                    ...
                    [#assign arg = ""]
                        ...
                        [#else] 
                            ...
                            [#if config.ipName=="PLAY" && configModel.usedDriver?? && configModel.usedDriver=="LL" && fargument.name?matches("input\\[LL_PLAY_LUT_INPUT\\d\\]")][#assign arg = "" + adr + "LL_PLAY_LUT_SOURCE_SWTRIG15"] [/#if]
                    [/#if]
                        [/#if] [#-- if genericType=Array --]
            ...
            [/#list]
[#if !(method.callMethod) || (S_FATFS_SDIO?? && (inst=="SDIO" || inst?starts_with("SDMMC") && (method.name)?starts_with("HAL_SD_")) && !(method.name=="HAL_RCCEx_PeriphCLKConfig"))] [#-- if HAL_SD_Init  and SDIO is used with FATFS--]
	[#if ((method.name)?starts_with("HAL_SD_") && (DIE == "DIE485" || FamilyName == "STM32H7"))]
		[#if method.returnHAL=="false"]
    		[#if nTab==2]#t#t[#else]#t[/#if]${method.name}();
	    [#else]
...
[#--[#if nTab==2]#t#t[#else]#t[/#if]${method.name}(${args});#n--]
        ...
        [#else]
            ...
            [#if !listofCalledMethod?contains(methodName) && method.optimizale??]
                [#if !method.name?matches("HAL_PLAY_(((IN|OUT)PUT)|(LUT))_SetConfig") || playInputs == 1]
                    [#if nTab==2]#t#t[#else]#t[/#if]if (${method.name}(${args}) != [#if method.returnHAL == "true"]${errorCode}[#else]${method.returnHAL}[/#if])
                    [#if nTab==2]#t#t[#else]#t[/#if]{
                    [#if nTab==2]#t#t[#else]#t[/#if]#tError_Handler();
                    ...
                    [#assign listofCalledMethod = listofCalledMethod + ", "+ methodName]
                    [/#if]
                    [#if method.name?matches("HAL_PLAY_(((IN|OUT)PUT)|(LUT))_SetConfig")]
                        [#assign playInputs = getPlayNumberOfMethods(configModel, method.name)]
                    [/#if]
                [#else]
                    #n
                    [#assign playInputs = playInputs - 1]
                [/#if]
...
#n
                ...
                [/#if]
                [#if method.name?matches("HAL_PLAY_(((IN|OUT)PUT)|(LUT))_SetConfig")]
                    [#assign playIndex = playIndex + 1]
                    [#assign playMethod = method.name]
                [/#if]
        [/#list]
[#-- assign instanceIndex = "" --]
 [#-- else there is no LibMethod to call--]
        ...
        [#list method.arguments as argument][#-- list method.arguments --]
            [#if argument.genericType == "struct"][#-- if struct --]
                ...
                [#if argument.context??][#-- if argument.context?? --]
                    ...
                    [#if !exist]  [#-- if exist --]                  
                    #t${argument.typeName} ${argument.name}[#if argument.typeName?contains("HAL_PLAY")][#if argument.typeName?contains("IN")][CONFIGURED_INPUT_NBR][/#if][#if argument.typeName?contains("OUT")][CONFIGURED_OUTPUT_NBR][/#if][#if argument.typeName?contains("LUT")][CONFIGURED_LUT_NBR][/#if][#else] = {0}[/#if];
                      [#assign myListOfLocalVariables = myListOfLocalVariables + " "+ argument.name]
                      [#assign resultList = myListOfLocalVariables]
                    [/#if][#-- if exist --]
                    [/#if][#-- if global --]
...
[#if tabN == 2]#t[/#if]#t/* Peripheral clock enable */
                    [#list IPData.initServices.clock?split(';') as clock]
                        ...
                        [#else]--]
                         [#if RESMGR_UTILITY?? && FamilyName=="STM32MP2" && isMP2_A35TD_Or_M33TDNonSecureClock(ipName)]
                           [#list configs as mxMcuDataModel]
                           [#assign index = 0]
                           [#assign FeatureIDUIIndex = index][#assign index += 1]
                           [#assign mx_RIF_Params = mxMcuDataModel.peripheralRIFParams]
        ...
        [/#list]
        [/#list] 	[/#if]
        [#if FamilyName!="STM32MP2" || isMP2_A35TD_Or_M33TDNonSecureClock(ipName)]
...
[#function getPlayNumberOfMethods(configModel, methodName)]
    [#assign number = 0]
    [#list configModel.configs as config]
        [#if config.methods??] [#-- if the pin configuration contains a list of LibMethods--]
            [#assign methodList = config.methods]
